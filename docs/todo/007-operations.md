...
- [ ] EAS Update(`expo-updates`) 도입 검토 — 카카오 로그인 배경 수정(2026-09-02)처럼
      **JS만 바뀐 수정도 지금은 스토어 심사를 거쳐야만 배포됨.** OTA가 있으면 몇 분 안에
      전 사용자에게 반영 가능. 도입 자체는 네이티브 의존성 추가라 스토어 빌드 1회가
      필요하므로, 다음 정기 릴리즈에 끼워 넣는 편이 효율적
